...
today = date.today()
# BELOW WIDGETS ARE FOR LOGIN WINDOW
firstw = Tk()
firstw.title("SCHOOL BILLING SYSTEM")
firstw.geometry("1600x1000+0+0")
firstw['background'] = '#856ff8'
label = Label(text="SCHOOL BILLING SYSTEM",
              font=("times new roman", 35), bg="black", fg="white")
...
def second():
    global secondw
    secondw = Tk()
    secondw.title("SCHOOL BILLING SYSTEM")
    ...
    def studentid():
        # STUDENT DETAILS WINDOW [ SHOWS ALL STUDENT'S DETAILS ]
        rot = Tk()
        rot.title("SCHOOL BILLING SYSTEM")
        rot.geometry("1600x1000+0+0")
        mainlabel = Label(rot, text="STUDENT DETAILS", font=(
            "times new roman", 35), bg="black", fg="white")
        mainlabel.pack(side=TOP, fill=X)
        rot['background'] = '#8BEEDE'
        chat1 = ttk.Treeview(rot, height=20, columns=(
            'name', 'sur', 'fee'), selectmode="extended")
        ...
        ttk.Style().configure("Treeview.Heading",
                              background="blue", foreground="palevioletRed1")
        ...
        # OBTAIN ALL DETAILS FROM STUDENTS DATABASE
        conn = sqlite3.connect("details.db")
        with conn:
            cur = conn.cursor()
            cur.execute('SELECT id ,name, fee ,  sur FROM kistar ')
            for row1 in cur.fetchall():
                chat1.insert('', 0, text=row1[0], values=(
                    row1[1], row1[2], row1[3]))

    def viewenquiry2():

        # ENQUIRY PAGE [ MAKE ALL ENQUIRIES WITH THE MANAGEMENT]

        rt = Tk()
        rt.title("SCHOOL BILLING SYSTEM")
        rt.geometry("1600x1000+0+0")
        mainlabel = Label(rt, text="VISITOR", font=(
            "times new roman", 35), bg="black", fg="white")
        rt['background'] = '#8BEEDE'
        mainlabel.pack(side=TOP, fill=X)
        chat1 = ttk.Treeview(rt, height=20, columns=(
            'EMAIL', 'ENQUIRY', 'DATE'), selectmode="extended")
        ...
        # SAVE ALL ENQUIRIES TO DATABASE
        conn = sqlite3.connect("details.db")
        with conn:
            cur = conn.cursor()
            cur.execute('SELECT * FROM golu')
            for row in cur.fetchall():
                chat1.insert('', 0, text=row[0],
                             values=(row[1], row[2], row[3]))

    # HOME PAGE THAT SHOWS ALL FEATURES
    def distroy5():
        secondw.destroy()
        window()
    mainlabel = Label(secondw, text="SCHOOL BILLING SYSTEM", font=(
        "times new roman", 35), bg="black", fg="white")
    mainlabel.pack(side=TOP, fill=X)

    secondw['background'] = 'grey'
    button = Button(secondw, width=15, font=("arial", 20),
                    text="REGISTRATION", bg="black", fg="white", command=distroy4)
    button.place(x=50, y=180)
    enquiry = Button(secondw, width=15, font=("arial", 20),
                     text="FEE DETAILS", bg="black", fg="white", command=distroy5)
    enquiry.place(x=320, y=280)
    fee_details = Button(secondw, width=15, font=(
        "arial", 20), text="ENQUIRY", bg="black", fg="white", command=enquiry1)
    fee_details.place(x=610, y=380)
    viewenquiry = Button(secondw, width=15, font=(
        "arial", 20), text="VIEW ENQUIRY", bg="black", fg="white", command=viewenquiry2)
    viewenquiry.place(x=890, y=480)
    viewenquiry1 = Button(secondw, width=15, font=(
        "arial", 20), text="STUDENT DETAILS", bg="black", fg="white", command=studentid)
    viewenquiry1.place(x=1150, y=580)
...
def login():
    if user.get() == "admin" and user3.get() == "admin":
        ...
        t = tkinter.messagebox.showinfo(
            "INVALID USERNAME OR PASSWORD ", "YOU HAVE ENTERED INVALID USERNAME OR PASSWORD  ")
        user.delete(0, END)
        user3.delete(0, END)
...
def root():
    ...
    calculation2 = 200000
    label = Label(root, text="REGISTRATION FORM", font=(
        "arial", 25), bg="black", fg="white")
    label.pack(side=TOP, fill=X)
    root['background'] = '#8BEEDE'
    ...
    entry5 = Entry(root, textvar=calculate, state="readonly",
                   width=20, font=("arial", 15, "bold"), bd=5)
    ...
    entry4 = Entry(root, bd=5, text="enter roll no.", width=20,
                   textvar=search, font=("arial", 15))
    entry4.place(x=800, y=150)
    search.set("")

    radio1 = Radiobutton(root, text="MALE", variable=gander,
                         value=1, font=("arial", 13))
    radio1.place(x=515, y=340)

    radio2 = Radiobutton(root, text="FEMALE", variable=gander,
                         padx=20, value=0, font=("arial", 13))
    radio2.place(x=590, y=340)
    gander.set(3)

    box = ttk.Combobox(root, textvariable=branch,
                       state="readonly", font=("arial", 12, "bold"), width=22)
    box['values'] = ['SELECT', 'COMPUTER SCIENCE', 'MACHENICAL', 'CIVIL', 'IT']
    box.current(0)
    box.place(x=503, y=395)

    checkbutton1 = Checkbutton(root, text="B.TECH", variable=java)
    checkbutton1.place(x=502, y=455)

    checkbutton1 = Checkbutton(root, text="M.TECH", variable=c)
    checkbutton1.place(x=582, y=455)

    checkbutton1 = Checkbutton(root, text="B.ED", variable=d)
    checkbutton1.place(x=662, y=455,)

    checkbutton1 = Checkbutton(root, text="PHD", variable=python)
    checkbutton1.place(x=742, y=455)
    python.set(0)
    java.set(0)
    c.set(0)
    d.set(0)
    ...
    button1 = Button(root, text="CALCULATE FEE", width=14, font=(
        "arial", 10), bg="black", fg="white", command=calculation)
    button1.place(x=530, y=630)

    button12 = Button(root, text="BACK", width=17, font=(
        "arial", 17), bg="red", fg="black", command=dis)
    button12.place(x=0, y=0)

    button2 = Button(root, text="SUBMIT FORM", width=14, font=(
        "arial", 10), bg="black", fg="white", command=msg)
    button2.place(x=660, y=630)

    button3 = Button(root, text="RESET", width=14, font=(
        "arial", 10), bg="black", fg="white", command=golu)
    button3.place(x=395, y=630)

    button4 = Button(root, text="SEARCH", width=14, font=(
        "arial", 10), bg="black", fg="white", command=all)
    button4.place(x=1100, y=150)

    button4 = Button(root, text="UPDATE", width=14, font=(
        "arial", 10), bg="black", fg="white", command=update)
    button4.place(x=950, y=630)

    button5 = Button(root, text="DELETE", width=14, font=(
        "arial", 10), bg="black", fg="white", command=delete)
...
def ka():
    ...
    with conn:
        cur = conn.cursor()
        cur.execute('INSERT INTO golu(NAME,PHONE,PURPOSE,DATE)VALUES(?,?,?,?)',
                    (NAMEE, PHONE, PURPOSE, today,))
        conn.commit()
...
def enquiry1():
    ...
    button = Button(enquiry1, text="submit", width=30,
                    bg="black", fg="white", command=r)
    button.place(x=500, y=320)
    button1 = Button(enquiry1, text="<< BACK", width=30,
                     bg="red", fg="black", command=enquiry1destroy)
    button1.place(x=0, y=0)
    entry24 = Entry(enquiry1, bd=5, width=20, font=("arial", 15))
    entry24.place(x=500, y=210)
    box2 = ttk.Combobox(enquiry1, textvariable=purpose,
                        state="readonly", font=("arial", 12, "bold"), width=22)
    box2['values'] = ['SELECT', 'HOSTEL COMPAINTS',
                      'EXAM DETAILS', 'FEE DETAILS']
    box2.current(0)
...
def cat():
    ...
    with conn:
        ...
        for row24 in cur.fetchall():
            ...
            cur.execute(
                ' SELECT SUM(INSTALLMENT) FROM FEEINSTALLMENT WHERE id=? GROUP BY id ', (FE,))
...
def fee_add():
    ...
    entry26.configure(state="normal")
    cur.execute(
        'INSERT INTO FEEINSTALLMENT(id , totel,INSTALLMENT, DATE)VALUES(?,?,?,?)', (FE, x, y, today,))
    cur.execute(
        ' SELECT SUM(INSTALLMENT) FROM FEEINSTALLMENT WHERE id=? GROUP BY id ', (FE,))
    for row23 in cur.fetchall():
        entry27.delete(0, END)
...
def installment2():
    if int(entry29.index("end")) > int(0):
        webbrowser.open("file:///C:/Users/santh/Desktop/SEPM PRACTICAL FINAL/index.html")
        fee_add()
    else:
        x = tkinter.messagebox.showinfo(
            "NO FEE ADDED", "YOU HAVE NOT ADDED ANY FEE ")
...
def fego1():
    webbrowser.open("file:///C:/Users/santh/Desktop/SEPM PRACTICAL FINAL/recipt.html")
...
def window():
    ...
    button = Button(main, text="BACK", width=30, bg="red",
                    fg="black", command=distroy3)
    button.place(x=0, y=0)
    main['background'] = '#8BEEDE'
    ...
    button22 = Button(main, text="LOGIN", width=26, font=(
        "arial", 10), bg="black", fg="white", command=cat)
    button22.place(x=400, y=310)
    button23 = Button(main, text="ADD FEE", width=26, font=(
        "arial", 10), bg="black", fg="white", command=installment2)
    button23.place(x=650, y=310)

    button230 = Button(main, text="RECIPT", width=26, font=(
        "arial", 10), bg="black", fg="white", command=fego1)
    button230.place(x=900, y=310)

    entry29 = Entry(main, bd=5, width=20, font=("arial", 15))
    entry29.place(x=650, y=200)
    button28 = Button(main, text="RESET", width=26, font=(
        "arial", 10), bg="red", fg="black", command=reset2)
...
def calculation():
    ...
    if NAME == ("") and SUR == ("") and EMAIL == ("") and BOX == ("SELECT") and GANDER == (3) and JAVA == (0) and PYTHON == (0) and C == (0) and D == (0):
        kal = tkinter.messagebox.showinfo(
            " DETAILS INVALID", "FILL ALL THE DETAILS")
...
def msg():
    ...
    if "@" and ".com" not in entry3.get():
        kal = tkinter.messagebox.showinfo(
            " INVALID DETAILS", "ENTER VALID EMAIL ADDRESS")
        entry3.delete(0, END)
    else:
        msg = tkinter.messagebox.askyesno(
            "Form filling confarmation", " WARNING: All data will be erase after 'YES' for new applicant")
        if msg > 0:
            NAME = entry1.get()
            SUR = entry2.get()
            EMAIL = entry3.get()
            ...
            with conn:
                cur = conn.cursor()
                cur.execute('INSERT INTO kistar (name,sur, email, branch, gander,fee ,python,java,c,d ) VALUES(?,?,?,?,?,?,?,?,?,?)',
...
def update():
    box1 = tkinter.messagebox.askyesno(
        "CONFIRMATION", "if you update you will be unable to see previous data again")
    if box1 > 0:
        SEARCH = entry4.get()
        NAME = entry1.get()
        SUR = entry2.get()
        ...
        with conn:
            ...
            cur.execute('UPDATE kistar SET email=? WHERE id=?',
                        (EMAIL, SEARCH,))
            cur.execute('UPDATE kistar SET branch=? WHERE id=?',
                        (BRANCH, SEARCH,))
            cur.execute('UPDATE kistar SET gander=? WHERE id=?',
                        (GANDER, SEARCH,))
            cur.execute('UPDATE kistar SET python=? WHERE id=?',
                        (PYTHON, SEARCH,))
...
def delete():
    box = tkinter.messagebox.askyesno(
        "WARNING", "DATA WILL NOT BE RECOVER AGAIN")
    if box > 0:
        SEARCH = entry4.get()
...
INQUIRY = Button(text="LOGIN", width=17, font=("arial", 20),
                 bg="black", fg="white", command=login)
